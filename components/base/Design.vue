<template>
  <div class="w-100 col-sm-6 col-md-3 d-flex flex-column p-3">
    <div class="bg-white rounded shadow-sm overflow-hidden flex-1 flex flex-col p-1">
      <nuxt-link  :to="{name: 'designs.detail', params:{slug: design.slug}}">
        <div class="bg-cover h-48" :style="`background-image: url('${design.images.original_image}')`">
      ...
      <div class="p-1 d-flex flex-column">
        <div class="mb-0 text-secondary d-flex justify-content-between align-items-center">
          <nuxt-link :to="{name: 'designs.detail', params:{slug: design.slug}}" class=" d-flex text-muted">
            <div>
              <img :src="design.user.avatar_url" width="20" class="profile rounded mr-2"/>
            </div>
            <h3 class="card-title font-13 mt-1 mb-2 fw-400">
              {{ design.user.name }}
              <span v-if="design.team" class="badge badge-pill badge-secondary text-white font-10 pr-1 pl-1 fw-300">+ team</span>
            </h3>
          </nuxt-link>
          ...
          <div class="ml-1 text-right flex-1">
            <ul class="font-12 fw-400 list-inline text-light-gray">
              <li class="pt-0 list-inline-item">
                <i class="fas fa-thumbs-up"></i>{{ design.like.count }}
              </li>
              <li class="pt-0 list-inline-item">
